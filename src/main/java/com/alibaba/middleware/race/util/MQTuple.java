...
public class MQTuple implements Serializable {
	...
	protected ArrayList<MessageWrapper> msgList;
	...
	public MQTuple(List<MessageExt> msgList, MessageQueue mq) {
		this.msgList = MQTuple.generateMessages(msgList);
		this.mq = mq;
	...
	public static  ArrayList<MessageWrapper> generateMessages(List<MessageExt> msgList){
		ArrayList<MessageWrapper> tmpList = new ArrayList<MessageWrapper>();
		
		for(MessageExt msg : msgList){
			tmpList.add(new MessageWrapper(msg.getTopic(),msg.getBody()));
		}
	...
	public List<MessageWrapper> getMsgList() {
		return msgList;
	}
